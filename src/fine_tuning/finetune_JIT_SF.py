from sklearn.metrics import roc_auc_score, auc, accuracy_score, precision_recall_fscore_support, precision_recall_curve, matthews_corrcoef
            ...
            if args.global_rank == 0 and \
                ...
                output_dir = os.path.join(args.output_dir,
                                          '{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}'.format(checkpoint_prefix, global_step,
                                                                           round(cur_res['accuracy'], 4),
                                                                           round(cur_res['f1'], 4),
                                                                           round(cur_res['precision'], 4),
                                                                           round(cur_res['recall'], 4),
                                                                           round(cur_res['auc_roc1'], 4),
                                                                           round(cur_res['auc_roc2'], 4),
                                                                            round(cur_res['auc_pr1'], 4),
                                                                           round(cur_res['auc_pr2'], 4),
                                                                            round(cur_res['mcc'], 4)))
                save_model(model, optimizer, scheduler, output_dir, config)
                f1 = cur_res["f1"]
                if f1 > best_f1:
...
def eval_metrics_epoch(args, eval_dataloader, model, tokenizer, show_bar=False, test=False):
    ...
    auc_roc1 = roc_auc_score(gold, probs)
    auc_roc2 = roc_auc_score(gold, pred)
    auc_pr1 = auc_pc(gold, probs)
    auc_pr2 = auc_pc(gold, pred)
    ...
    if test is True:
        ...
        # res["E20R"] = E20R
        # res["Popt"] = Popt
        df = pd.DataFrame({'label': gold, 'pred': probs})
        save_path = "{}/checkpoint-best-f1/test_com.csv".format(args.output_dir)
        df.to_csv(save_path, index=False, sep=',')

    res["auc_roc1"] = auc_roc1
    res["auc_roc2"] = auc_roc2
    res["auc_pr1"] = auc_pr1
    res["auc_pr2"] = auc_pr2
